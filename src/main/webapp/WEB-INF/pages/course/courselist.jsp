...
<nav class="breadcrumb"> <i class="Hui-iconfont">&#xe67f;</i> 首页
	...
	查看所有课程<a class="btn btn-success radius r mr-20"
...
<div class="mt-20">
 ...
 <div style="text-align:center;font-size: 20px;height: 50px;">
  ...
  <a href="<%=basePath%>/courses/getall?pagenum=1">首页</a>  
  
  
  <c:if test="${infos.hasPreviousPage }">
   <a href="<%=basePath%>/courses/getall?pagenum=${infos.pageNum-1}">上一页</a>
  </c:if>
    
      
  <c:forEach items="${infos.navigatepageNums}" var="pagenumber">
  <a href="<%=basePath%>/courses/getall?pagenum=${pagenumber}">${pagenumber}</a> 
  </c:forEach>
  
  
  <c:if test="${infos.hasNextPage }">
   <a href="<%=basePath%>/courses/getall?pagenum=${infos.pageNum+1}">下一页</a>
   </c:if>
  

   
    <a href="<%=basePath%>/courses/getall?pagenum=${infos.pages}">末页</a>   
