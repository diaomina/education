...
<body>
	...
	<span class="c-gray en">&gt;</span> 课程测验 <span class="c-gray en">&gt;</span>
	查看以往成绩 <!-- <a class="btn btn-success radius r mr-20"
	...
	<div class="pd-20">
		<div class="text-c">
			...
			<%-- <form action="${webpath}/paperAnswer/toPaperAnswer2" method="post">
		...
		<!-- 	<input type="submit" value="查询" />	 -->
			<input type="text" class="input-text" style="width: 100px"
				placeholder="学生学号" id="" name="stuId" oninput="value=value.replace(/[^\d]/g,'')">
			...
			<table
				...
				<thead>
					<tr class="text-c">
						...
						<th width="130">查看答题情况</th>
					</tr>
				</thead>
				
				<c:forEach items="${lists}" var="paperScore">
					<tbody>
						<tr class="text-c">
		...
		<div style="width: 100%;margin-left: 10px">
			...
			<div style="margin-left:92%;margin-top: -20px"><a
				href="${webpath}/paperScore/toStudentScore?pagenum=1">首页</a>

			
			<c:if test="${infos.hasPreviousPage }">
				<a
					href="${webpath}/paperScore/toStudentScore?pagenum=${infos.pageNum-1}">上一页</a>
			</c:if>


			<c:forEach items="${infos.navigatepageNums}" var="pagenumber">
				<a href="${webpath}/paperScore/toStudentScore?pagenum=${pagenumber}">${pagenumber}</a>
			</c:forEach>


			<c:if test="${infos.hasNextPage }">
				<a
					href="${webpath}/paperScore/toStudentScore?pagenum=${infos.pageNum+1}">下一页</a>
			</c:if>



			<a href="${webpath}/paperScore/toStudentScore?pagenum=${infos.pages}">末页</a>
</div>
		</div>
	</div>
